...
- 將完整編輯狀態儲存為 `.pixel-art.json`，之後可載入繼續編輯
- 將完成的作品匯出為 PNG
- 內建使用說明視窗與響應式操作介面
...
點擊頁面頂部的儲存圖示，可將目前作品下載為 `.pixel-art.json` 專案檔。專案檔會保存：
...
點擊資料夾圖示並選擇先前儲存的專案檔，或將 `.pixel-art.json` 直接拖放到中央畫布區域，即可恢復內容及設定並繼續編輯。無效的專案檔不會覆蓋目前作品，畫面會顯示錯誤提示。

> PNG 適合分享最後成果；`.pixel-art.json` 適合保存仍要繼續編輯的專案。
...
├── utils/               # 像素化、Canvas 快照、專案存取與 PNG 匯出工具
├── App.tsx              # 應用程式狀態與主要版面
